...
module.exports = {
    ...
    run: async (msg, args, client) => {
        ...
        else if (args[0]=="Config"||args[0]=="config")
        {
            const EmbedPomoc = new MessageEmbed()
            .setColor("RED")
            .setDescription("`ustaw` - Ustawia kanał powitania i pożegnania!\n `logi` - **( BETA )** Ustawia kanał logów!\n`antybot` - **WKRÓTCE** \n `weryfikacja` - **WKRÓTCE**\n\n > Wiemy że nie każdy ogarnia naszego bota posiada on dużo argumentów itp. Więc stworzyliśmy [dokumentacje](https://solindeklive-biznes.gitbook.io/shield-bot/) oraz [kanał na youtube](http://www.huj.pl) poświęcony naszemu botowi!")
